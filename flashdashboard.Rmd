...
tocompare2 <- reactive({
  ...
  mutate(across(where(is.numeric), rescale, na.rm = TRUE)) %>%
  tidyr::pivot_longer(cols = c("Crossing":"GKReflexes"), names_to = "Feature", values_to = "value") %>%
  dplyr::group_by(Role, Club, Feature) %>%
  dplyr::summarise(value = mean(value)) %>%
  tidyr::pivot_wider(names_from = "Feature", values_from = "value")
  return(data)
})
...
ggRadar(data=tocompare2(), aes(colour=Role, facet=Club), rescale = FALSE, 
        interactive=TRUE, use.label=FALSE, 
        legend.position = "right", alpha=0.2, size=0.5)
)
